...
import { CourseItem, DEFAULT_STORAGE, PageAnimationType, StorageData } from "../def"
...
const EditModal = ({ open, date, resourceId, setOpen, setPath, setStorage }: EditModalProps) => {
            ...
            <ModalBody pb='50px' pt='50px'>
                ...
                <VStack>
                    ...
                    {newDate.getTime() != date.getTime() && <Button bg='black' colorScheme='blackAlpha' w='100%' onClick={()=>{
                        setOpen(false);
                        setPath(`/planning/${resourceId}/${newDate.getTime()}`, PageAnimationType.SPEED)
                    }}>Valider</Button>}
                    <Divider mt='10px' mb='10px' />
                    <Button w='100%' colorScheme='gray' onClick={() => {
                        setOpen(false);
                        setPath("/selection", PageAnimationType.REVERSE);
